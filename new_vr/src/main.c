...
int ISMupdate(void)
{
  ...
  if (ret) {
    	...
    	if ( abs(pos.scroll ) >  abs(ISM_gyro_angle.r) && abs(ISM_gyro_angle.p) >  abs(ISM_gyro_angle.y) ) {
...
{
	static uint8_t battery_notify = 0;

	battery_notify++;
	if (battery_notify >= 100) {
		battery_notify = 0;
		bas_notify();
	}
}
