...
*/
	k_msleep(100);
	/*
	ASKA_write_reg(ASKA_ELE1,0x00008000);
	ASKA_write_reg(ASKA_ELE2,0x00004000);
	ASKA_write_reg(ASKA_CONF0,0x19672190);
	ASKA_write_reg(ASKA_CONF1,0x00906420);
	*/
	while (1) {
		gpio_pin_toggle_dt(&led);

	// 50mA 0.5 ramp, 0.5 ON, 0.5 OFF, 4 PD

		//Disable ASKA before reprogram!
		ASKA_write_reg(ASKA_CONF1,0x00000000); 
		//new peogram
		ASKA_write_reg(ASKA_ELE1,0x00008000);
		ASKA_write_reg(ASKA_ELE2,0x00004000);
		ASKA_write_reg(ASKA_CONF0,0x19672190);
	...
	// 25mA 1s ramp, 1s ON, 1s OFF, 4 PD
		...
		ASKA_write_reg(ASKA_ELE1,0x00008000);
		ASKA_write_reg(ASKA_ELE2,0x00004000);
		ASKA_write_reg(ASKA_CONF0,0x32c99190);
		ASKA_write_reg(ASKA_CONF1,0x0090c808);
